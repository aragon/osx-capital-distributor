...
contract ActionEncoderFactoryTest is Test {
    ...
    function test_RegisterActionEncoder_RevertInvalidInterface() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(FactoryBase.InvalidImplementation.selector, invalidImplementation, "Implementation must support IPayoutActionEncoder interface"));
    ...
    function test_DeployActionEncoder_RevertInstanceAlreadyDeployed() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(FactoryBase.InstanceAlreadyDeployed.selector, deploymentId, address(encoder1)));
        factory.deployActionEncoder(VAULT_ENCODER_ID, dao, auxData);
    }
...
contract ActionEncoderMock is PayoutActionEncoderBase {
    ...
    function buildActions(
        ...
        uint256,
        bytes calldata
    ) external pure override returns (Action[] memory actions) {
