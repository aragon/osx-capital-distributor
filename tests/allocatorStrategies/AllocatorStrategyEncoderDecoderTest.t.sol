...
contract AllocatorStrategyEncoderDecoderTest is Test {
    ...
    // ============================================
    // GaugeDistributionStrategy Tests
    // ============================================
    
    /// @notice Test encoding/decoding of initialization params for GaugeDistributionStrategy
    function test_GaugeStrategy_InitializationParams() public {
        address gaugeVoter = makeAddr("gaugeVoter");
        
        // Encode
        bytes memory encoded = gaugeStrategy.encodeInitializationParams(IAddressGaugeVoter(gaugeVoter));
        
        // Decode
        IAddressGaugeVoter decoded = gaugeStrategy.decodeInitializationParams(encoded);
        
        assertEq(address(decoded), gaugeVoter, "Decoded gauge voter should match original");
    }
    
    /// @notice Test encoding/decoding of campaign creation params for GaugeDistributionStrategy
    function test_GaugeStrategy_SetAllocationCampaignParams() public {
        uint256 totalDistribution = 10000 ether;
        
        // Encode
        bytes memory encoded = gaugeStrategy.encodeSetAllocationCampaignParams(totalDistribution);
        
        // Decode
        uint256 decoded = gaugeStrategy.decodeSetAllocationCampaignParams(encoded);
        
        assertEq(decoded, totalDistribution, "Decoded distribution amount should match original");
    }
    
    /// @notice Test encoding/decoding of claim params for GaugeDistributionStrategy
    function test_GaugeStrategy_ClaimParams() public {
        // GaugeDistributionStrategy doesn't use claim params
        bytes memory encoded = gaugeStrategy.encodeClaimParams();
        assertEq(encoded.length, 0, "Gauge claim params should be empty");
    }
    
    /// @notice Fuzz test for GaugeDistributionStrategy initialization params
    function testFuzz_GaugeStrategy_InitializationParams(address gaugeVoter) public {
        vm.assume(gaugeVoter != address(0)); // Avoid zero address
        
        bytes memory encoded = gaugeStrategy.encodeInitializationParams(IAddressGaugeVoter(gaugeVoter));
        IAddressGaugeVoter decoded = gaugeStrategy.decodeInitializationParams(encoded);
        
        assertEq(address(decoded), gaugeVoter, "Fuzz: decoded gauge voter should match");
    }
    
    /// @notice Fuzz test for GaugeDistributionStrategy campaign params
    function testFuzz_GaugeStrategy_SetAllocationCampaignParams(uint256 totalDistribution) public {
        bytes memory encoded = gaugeStrategy.encodeSetAllocationCampaignParams(totalDistribution);
        uint256 decoded = gaugeStrategy.decodeSetAllocationCampaignParams(encoded);
        
        assertEq(decoded, totalDistribution, "Fuzz: decoded distribution should match");
    }
    ...
    function test_ManualEncodingCompatibility() public {
        // Test MerkleDistributorStrategy
        bytes32 merkleRoot = keccak256("compatibility-test");
        bytes memory manualEncoded = abi.encode(merkleRoot);
        bytes memory functionEncoded = merkleStrategy.encodeSetAllocationCampaignParams(merkleRoot);
        assertEq(manualEncoded, functionEncoded, "Manual and function encoding should match");
        
        // Test GaugeDistributionStrategy
        address gaugeVoter = makeAddr("compatibility-gauge");
        manualEncoded = abi.encode(IAddressGaugeVoter(gaugeVoter));
        functionEncoded = gaugeStrategy.encodeInitializationParams(IAddressGaugeVoter(gaugeVoter));
        assertEq(manualEncoded, functionEncoded, "Manual and function encoding should match for gauge");
    }
