...
contract VaultDepositPayoutActionEncoderTest is AragonTest {
    ...
    function test_SetupCampaign_Success() public {
        ...
        emit CampaignVaultSet(CAMPAIGN_ID, newVault, encoder.owner());
    ...
    function test_SetupCampaign_RevertNotOwner() public {
        address newVault = makeAddr("newVault");
    ...
    function test_BuildActions_CorrectApprovalAction() public {
        ...
        Action[] memory actions =
            encoder.buildActions(mockToken, testAlice, DEFAULT_AMOUNT, address(this), CAMPAIGN_ID, bytes(""));
        ...
        (address spender, uint256 approveAmount) =
    ...
    function test_BuildActions_CorrectDepositAction() public {
        ...
        // Extract parameters
        (uint256 depositAmount, address depositRecipient) =
    ...
    function test_BuildActions_DifferentCampaigns() public {
        ...
        Action[] memory actions1 =
            encoder.buildActions(mockToken, testAlice, DEFAULT_AMOUNT, address(this), CAMPAIGN_ID, bytes(""));

        // Build actions for campaign 2
        Action[] memory actions2 =
            encoder.buildActions(mockToken, testBob, DEFAULT_AMOUNT * 2, address(this), campaign2, bytes(""));
    ...
    function testFuzz_BuildActions_Amounts(uint256 amount) public {
        ...
        Action[] memory actions =
            encoder.buildActions(mockToken, testAlice, amount, address(this), CAMPAIGN_ID, bytes(""));
        ...
        (uint256 depositAmount,) = abi.decode(slice(actions[1].data, 4, actions[1].data.length - 4), (uint256, address));
    ...
    function test_FactoryIntegration_FullFlow() public {
        ...
        // Setup campaign
        vm.prank(newEncoder.owner());
        newEncoder.setupCampaign(10, auxData);

        // Build actions
        Action[] memory actions = newEncoder.buildActions(mockToken, testAlice, 500 ether, address(this), 10, bytes(""));
    ...
    function test_LargeAmounts() public {
        ...
        Action[] memory actions =
            encoder.buildActions(mockToken, testAlice, maxAmount, address(this), CAMPAIGN_ID, bytes(""));
...
contract MockDAO is IDAO {
    ...
    function deposit(address, uint256, string calldata) external payable { }
    ...
    function registerStandardCallback(bytes4, bytes4, bytes4) external { }
