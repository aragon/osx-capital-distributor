...
I want you to write an in-depth test suite for the AllocatorStrategyFactory. Assume the FactoryBase is already tested,
so you don't have to do that already.

You can use mocks for some of the contracts in the registry, but there's also some inside the allocatorStrategies/
directory. The most important one is the MerkleDistributor.

This contract serves as a registry and factory for allocator strategies, that is, contracts that following an interface
(you can find it in the interfaces directory) allocate some funds to an address. Each implementation may have its own
logic to decide how to do that.
